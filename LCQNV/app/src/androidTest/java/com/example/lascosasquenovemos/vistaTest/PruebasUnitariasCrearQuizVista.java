...
public class PruebasUnitariasCrearQuizVista implements QuizListener {
    ...
    public void onQuizWriteSucced(Boolean bool) {
        if (bool == true) onView(withId(R.id.ErrorQuiz)).perform(setTextInTextView("CREADO CON ÉXITO"));
        else onView(withId(R.id.ErrorQuiz)).perform(setTextInTextView("ERROR AL CREAR"));
    }
}
